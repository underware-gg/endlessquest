...
library Crawl {
  ...
  //
	...
	function tokenName(string memory tokenId) internal pure returns (string memory) {
		return string.concat("Chamber #", tokenId);
	}

	/// @dev Short string representation of a Chamebr coordinate and Yonder
	function coordsToString(uint256 coord, uint256 yonder, string memory separator) internal pure returns (string memory) {
